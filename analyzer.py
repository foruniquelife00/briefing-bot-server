...
## 📝 브리핑 형식 (📰 오늘의 시장 배경)
0. 맨 첫 줄에 반드시 아래를 그대로 출력한다 (변형 금지):
   "📖 이 브리핑은 '오늘 시장의 배경'입니다 — 종목 판단은 07:00 시그널봇이 합니다."
   ★ 2026-09-04 신설: 사용자가 브리핑봇과 시그널봇의 차이를 구분하지 못하는 문제가
     있었다. 머리(이 줄)와 꼬리(7번 섹션) 양쪽에서 역할을 명시한다.
...
7. 🔗 시그널봇 연결
   - 반드시 아래 두 줄을 그대로 출력한다 (시각·표현 임의 변경 금지):
     "지금까지는 오늘 시장의 '배경'입니다."
     "곧이어 07:00 시그널봇이 수급 기준 관심목록과 시장상태를 보냅니다 — 종목 판단은 그쪽입니다."
   ★ 2026-09-04 수정: 이전에는 "08:00"으로 안내했으나 시그널봇 실제 발송은 07:00이다
     (deploy/systemd/signal-morning.timer — OnCalendar=Mon-Fri 07:00 Asia/Seoul).
     두 봇의 역할 차이가 사용자에게 드러나도록 '배경 / 판단' 구분을 명시한다."""
